...
let {isEmpty, isValidName, isValidPhone, isValidpincode, isValidObjectId, isValidStreet,isValidEmail} = validation;
...
let userLogin = async(req,res)=>{
  try{
    ...
    if(!email){
      res.status(400).send({status:false,msg:"please enter a user name"});
    }
    if(!password){
      res.status(400).send({status:false,msg:'please enter a password'});
    }
    let emailExist = await userModel.findOne({email:email});
    if(!emailExist){
      res.status(400).send({status:false,msg:'please provide correct email'});
    }
    let checkPassword = await bcrypt.compare(password, emailExist.password);
    if (!checkPassword) {
  ...
  return res.status(200).send({status: true,message: "User Login Successful",data: {userId: emailExist._id, token: token},});
  }catch(error){
...
module.exports.updateUsersProfile=updateUsersProfile